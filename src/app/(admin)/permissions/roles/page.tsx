      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-3">
          <Badge variant="outline" className="px-3 py-1">
            <FileText className="mr-1 h-3 w-3" /> {permissions.length} Total
            Permissions
          </Badge>
          <Badge variant="outline" className="px-3 py-1">
      ...
      <div className="grid grid-cols-2 gap-2">
        {roles.map(
          ({
            ...
            <Card
              key={role}
              className={`hover:shadow-md transition-shadow overflow-hidden rounded-sm  ${color.includes("border-") ? color.split(" ").find((c) => c.startsWith("border-")) : ""}`}
            >
              <div className="flex flex-col md:flex-row">
                <div
                  className={`hidden md:flex items-center justify-center p-6 ${color}`}
                  style={{ width: "100px" }}
                >
                  {icon}
                </div>
